 ...
 * @since Mar 2, 2004
 * @version $Id: Hyperlink.java,v 1.15 2009/07/23 04:12:00 alexpkb Exp $
 */
...
public final class Hyperlink extends Canvas {
	...
	public Hyperlink(final Composite parent, final int style) {
		...
		if (shell != null) {
			...
			{
				public void handleEvent(final Event event) {
					boolean newCursorInControl = getDisplay()
							.getCursorControl() == Hyperlink.this;
	...
	private boolean isInClientArea(final Event event) {
	...
	private Point getTotalSize(final String text) {
	...
	private void onPaint(final PaintEvent event) {
		...
		int uy = (rect.y + (rect.height - textHeight) / 2)
				...
				+ gc.getFontMetrics().getLeading() + 1;
	...
	public void setHoverForeground(final Color color) {
	...
	public void setUnderline(final Color color) {
		this.normalUnderline = color;
		redraw();
	}

	public void setHoverUnderline(final Color color) {
		this.hoverUnderline = color;
		redraw();
	}

	public void setActiveUnderline(final Color color) {
	...
	/**
	 * Set the Hyperlink's displayed text. The value <code>null</code> clears
	 * it.
	 * <p>
