...
/**
 * A non-native size grip which looks (and almost feels) like the native Win32
 * size grip.
 * <p>
 * The SHADOW_IN style causes highlight lines to be drawn at the right and
 * bottom border. This style should be used when placing the size grip on top of
 * the bottom right corner of a FramedComposite with style SHADOW_IN. If the
 * FLAT style is set, the size grip is drawn in a Windows XP style instead of
 * the normal Windows Classic style.
 * </p>
 * <p>
 * <dl>
 * <dt><b>Styles:</b></dt>
 ...
 * </p>
 * <p>
 * NOTE: The visibility of this widget is controlled by the "maximized" state of
 * the shell. The size grip is hidden when the shell is maximized, even if it
 * has been made visible by calling <code>setVisible(true)</code>. getVisible()
 * always returns the value set with setVisible(). isVisible() returns the true
 * visibility, as usual.
 * </p>
 * 
 * <p>
 * New in 1.6: Smoother resizing for a more native look & feel. The window size
 * is not updated more than once every 25ms to reduce the number of unnecessary
 * repaints.
 * </p>
 * 
 * <p>
 * New in 1.8: You can specify a parent Composite other than the shell which
 * will be resized by the SizeGrip.
 * </p>
 * 
 * @author Stefan Zeiger (szeiger@novocode.com)
 * @since Mar 8, 2004
 * @version $Id: SizeGrip.java 306 2005-01-24 21:52:21 +0000 (Mon, 24 Jan 2005)
 *          szeiger $
 */
...
public final class SizeGrip extends Canvas {
	...
	private final Composite resizableParent;

	public SizeGrip(final Composite parent, final int style) {
		this(parent, parent.getShell(), style);
	}

	public SizeGrip(final Composite parent, final Composite resizableParent,
			int style) {
	...
	private void performResize(final Event event) {
	...
	/**
	 * Set the allowed minimum size for the shell. The SizeGrip will not resize
	 * the shell to a smaller size.
	 * <p>
	 * Note: This does <em>not</em> affect other ways of resizing the shell,
	 * like using the size controls which are placed on the trimmings by the
	 * window manager.
	 * </p>
	 */

	public void setMinimumShellSize(final Point p) {
		checkWidget();
		this.minSize = p;
	}
	...
	public void setMinimumShellSize(final int width, final int height) {
		checkWidget();
