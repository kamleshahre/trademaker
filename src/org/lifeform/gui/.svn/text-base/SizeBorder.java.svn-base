 ...
 * 
 * <p>
 * If the style SWT.BORDER ist set, a beveled border (as used on Windows Classic
 * window decorations) will be drawn. Without this style, no drawing is done.
 * </p>
 * 
 * @author Stefan Zeiger (szeiger@novocode.com)
 * @since Jan 21, 2005
 * @version $Id: SizeBorder.java 321 2005-02-26 15:44:24 +0000 (Sat, 26 Feb
 *          2005) szeiger $
 */
...
public class SizeBorder extends Canvas {
	...
	private final Cursor cursorNWSE, cursorNESW, cursorWE, cursorNS;
	private int currentArea;

	public SizeBorder(final Composite parent, final int style) {
		this(parent, parent.getShell(), style);
	}

	public SizeBorder(final Composite parent, final Composite resizableParent,
			final int style) {
	...
	private void updateCursor(final int area) {
		Cursor c = null;
		switch (area) {
		case AREA_N:
		case AREA_S:
	...
	private int areaAtPoint(final int x, final int y) {
		Point size = getSize();
	...
	public void setCornerSize(final int size) {
		checkWidget();
		cornerSize = size;
		Point p = toControl(display.getCursorLocation());
		updateCursor(areaAtPoint(p.x, p.y));
