 ...
 * the LiveSashForm. The relative sizes of the children may be specfied using
 * weights.
 * <p>
 ...
 * 
 * @since Feb 15, 2004
 * @version $Id: LiveSashForm.java 338 2005-06-04 19:28:23 +0000 (Sat, 04 Jun
 *          2005) szeiger $
 */
...
public class LiveSashForm extends Composite {
	...
	private final static String LAYOUT_RATIO = LiveSashForm.class.getName()
			+ ".layoutRatio"; //$NON-NLS-1$
	private final static String CHILD_SHADOW = LiveSashForm.class.getName()
			+ ".childShadow"; //$NON-NLS-1$
	...
	public Point computeSize(final int wHint, final int hHint,
		...
		int sashwidth = sashes.length > 0 ? sashWidth
				+ sashes[0].getBorderWidth() * 2 : sashWidth;
		int width = 0;
		int height = 0;
	...
	public void layout(final boolean changed) {
		...
		if (maxControl != null && !maxControl.isDisposed()) {
			for (int i = 0; i < controls.length; i++) {
				if (controls[i] != maxControl) {
					...
					controls[i].setBounds(area.x + sh, area.y + sh, area.width
							- 2 * sh, area.height - 2 * sh);
				}
			}
			return;
		}
		...
		if (orientation == SWT.HORIZONTAL) {
			...
			controls[0].setBounds(x + sh, area.y + sh, width - 2 * sh,
					area.height - 2 * sh);
			x += width;
			for (int i = 1; i < controls.length - 1; i++) {
				int prevSh = sh;
				sh = shadowSizeForShadow(getChildBorder(controls[i]));
				sashes[i - 1].setBounds(x - prevSh, area.y, sashwidth + prevSh
						+ sh, area.height);
				x += sashwidth;
				width = (int) (ratios[i] * area.width / total);
				controls[i].setBounds(x + sh, area.y + sh, width - 2 * sh,
						area.height - 2 * sh);
				x += width;
			}
			if (controls.length > 1) {
				...
				sashes[sashes.length - 1].setBounds(x - prevSh, area.y,
						sashwidth + prevSh + sh, area.height);
				x += sashwidth;
				width = area.width - x;
				controls[controls.length - 1].setBounds(x + sh, area.y + sh,
						width - 2 * sh, area.height - 2 * sh);
			}
		} else {
			int height = (int) (ratios[0] * area.height / total);
			int y = area.y;
			int sh = shadowSizeForShadow(getChildBorder(controls[0]));
			controls[0].setBounds(area.x + sh, y + sh, area.width - 2 * sh,
					height - 2 * sh);
			y += height;
			for (int i = 1; i < controls.length - 1; i++) {
				int prevSh = sh;
				sh = shadowSizeForShadow(getChildBorder(controls[i]));
				sashes[i - 1].setBounds(area.x, y - prevSh, area.width,
						sashwidth + prevSh + sh);
				y += sashwidth;
				height = (int) (ratios[i] * area.height / total);
				controls[i].setBounds(area.x + sh, y + sh, area.width - 2 * sh,
						height - 2 * sh);
				y += height;
			}
			if (controls.length > 1) {
				int prevSh = sh;
				sh = shadowSizeForShadow(getChildBorder(controls[controls.length - 1]));
				sashes[sashes.length - 1].setBounds(area.x, y - prevSh,
						area.width, sashwidth + prevSh + sh);
				y += sashwidth;
				height = area.height - y;
				controls[controls.length - 1].setBounds(area.x + sh, y + sh,
						area.width - 2 * sh, height - 2 * sh);
			}
	...
	private void drawBorderAround(final Control c, final GC gc) {
		...
		switch (sh) {
		case SWT.SHADOW_IN:
			drawBevelRect(gc, r.x - 1, r.y - 1, r.width + 1, r.height + 1,
					shadow, highlight);
			break;
		...
		case SWT.SHADOW_ETCHED_IN:
			drawBevelRect(gc, r.x - 1, r.y - 1, r.width + 1, r.height + 1,
					highlight, shadow);
			drawBevelRect(gc, r.x - 2, r.y - 2, r.width + 3, r.height + 3,
					shadow, highlight);
			break;
	...
	private void onDragSash(final Event event) {
		...
		if (event.detail == SWT.DRAG) {
			Rectangle area = getClientArea();
			if (orientation == SWT.HORIZONTAL) {
				event.x = Math.min(Math.max(dragMinimum - sh1, event.x),
						area.width - dragMinimum - sashWidth - sh1);
			} else {
				event.y = Math.min(Math.max(dragMinimum - sh1, event.y),
						area.height - dragMinimum - sashWidth - sh1);
			}
			if (!liveUpdate)
				return;
		}
		...
		b1 = new Rectangle(b1.x - sh1, b1.y - sh1, b1.width + 2 * sh1,
				b1.height + 2 * sh1);
		Rectangle b2 = c2.getBounds();
		b2 = new Rectangle(b2.x - sh2, b2.y - sh2, b2.width + 2 * sh2,
				b2.height + 2 * sh2);
		...
		if (orientation == SWT.HORIZONTAL) {
			...
			Long c1new = new Long((((long) b1.width << 16) + area.width - 1)
					/ area.width);
			...
			c2.setData(LAYOUT_RATIO, new Long((((long) b2.width << 16)
					+ area.width - 1)
					/ area.width));
		} else {
			...
			Long c1new = new Long((((long) b1.height << 16) + area.height - 1)
					/ area.height);
			...
			c1.setData(LAYOUT_RATIO, c1new);
			c2.setData(LAYOUT_RATIO, new Long((((long) b2.height << 16)
					+ area.height - 1)
					/ area.height));
		}

		c1.setBounds(b1.x + sh1, b1.y + sh1, b1.width - 2 * sh1, b1.height - 2
				* sh1);
		sash.setBounds(event.x, event.y, event.width, event.height);
		c2.setBounds(b2.x + sh2, b2.y + sh2, b2.width - 2 * sh2, b2.height - 2
				* sh2);
	}
	...
	public void setWeights(final int[] weights) {
		...
		for (int i = 0; i < cArray.length; i++) {
			cArray[i].setData(LAYOUT_RATIO, new Long((((long) weights[i] << 16)
					+ total - 1)
					/ total));
		}
	...
	private Control createSash() {
		int sashStyle = (orientation == SWT.HORIZONTAL) ? SWT.VERTICAL
				: SWT.HORIZONTAL;
