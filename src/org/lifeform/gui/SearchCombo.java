...
public class SearchCombo extends Composite {
	 ...
	 * @see Widget#getStyle()
	 */
	public SearchCombo(Composite parent, int style) {
		...
		listener = new Listener() {
			public void handleEvent(Event event) {
				...
				if (SearchCombo.this == event.widget) {
					comboEvent(event);
					return;
				}
		...
		filter = new Listener() {
			public void handleEvent(Event event) {
				...
				if (shell == SearchCombo.this.getShell()) {
					handleFocus(SWT.FocusOut);
				}
			}
		};
	...
	Label getAssociatedLabel() {
		...
		for (int i = 0; i < siblings.length; i++) {
			if (siblings[i] == SearchCombo.this) {
...
//	}
	...
	public static void main(String[] args){
		...
		shell.setText("Search Combo");
		
		Composite parent = new Composite(shell, SWT.NONE);
		parent.setLayout(new FillLayout());


		SearchCombo cmb = new SearchCombo(parent, SWT.FLAT);
